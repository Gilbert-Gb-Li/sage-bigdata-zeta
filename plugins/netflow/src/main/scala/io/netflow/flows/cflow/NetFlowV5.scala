...
object NetFlowV5Packet {
 ...
 if (count <= 0 || buf.readableBytes < headerSize + count * flowSize){
    ...
    val flows: List[NetFlowV5] = (0 until count).toList.flatMap { i =>
      apply(sender, buf.slice(headerSize + (i * flowSize), flowSize), id, uptime, timestamp, samplingInterval)
    }
    NetFlowV5Packet(id, sender, buf.readableBytes, uptime, new Date(timestamp.getMillis), flows, flowSequence, engineType, engineId, samplingInterval, samplingMode)
  }
  ...
  def apply(sender: InetSocketAddress, buf: ByteBuf, fpId: UUID, uptime: Long, timestamp: DateTime, samplingInterval: Int): Option[NetFlowV5] =
    Try[NetFlowV5] {
     // val sampling = NodeConfig.values.netflow.calculateSamples
      ...
      NetFlowV5(UUID.randomUUID(), sender, buf.readableBytes(), uptime, new Date(timestamp.getMillis),
        ...
        if (samplingInterval>0) pkts * samplingInterval else pkts, // pkts
        if (samplingInterval>0) bytes * samplingInterval else bytes, // bytes
...
case class NetFlowV5Packet(id: UUID, sender: InetSocketAddress, length: Int, uptime: Long, timestamp: Date, flows: List[NetFlowV5],
...
case class NetFlowV5(id: UUID, sender: InetSocketAddress, length: Int, uptime: Long, timestamp:Date,
