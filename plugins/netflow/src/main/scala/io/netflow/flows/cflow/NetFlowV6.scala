...
object NetFlowV6Packet {
  ...
  def apply(sender: InetSocketAddress, buf: ByteBuf): Try[NetFlowV6Packet] = Try[NetFlowV6Packet] {
    ...
    NetFlowV6Packet(id, sender, buf.readableBytes, uptime, new Date(timestamp.getMillis), flows, flowSequence, engineType, engineId,
  ...
  def apply(sender: InetSocketAddress, buf: ByteBuf, fpId: UUID, uptime: Long, timestamp: DateTime, samplingInterval: Int): Option[NetFlowV6] =
    Try[NetFlowV6] {
     // val sampling = NodeConfig.values.netflow.calculateSamples
      ...
      NetFlowV6(UUID.randomUUID(), sender, buf.readableBytes(), uptime, new Date(timestamp.getMillis),
...
case class NetFlowV6Packet(id: UUID, sender: InetSocketAddress, length: Int, uptime: Long, timestamp: Date,
...
case class NetFlowV6(id: UUID, sender: InetSocketAddress, length: Int, uptime: Long, timestamp: Date,
