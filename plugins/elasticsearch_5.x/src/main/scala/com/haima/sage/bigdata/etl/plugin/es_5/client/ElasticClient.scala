package com.haima.sage.bigdata.etl.plugin.es_5.client
...
class ElasticClient private(name: String, host_ports: Array[(String, Int)]) {
  ...
  private val settings: Settings = Settings.builder().put("cluster.name", name)
  ...
  private def create(): TransportClient = {
    ...
    host_ports.foldLeft(new PreBuiltTransportClient(settings): TransportClient) {
