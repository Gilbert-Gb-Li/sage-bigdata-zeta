package com.haima.sage.bigdata.etl.plugin.es_2.writer
...
import com.haima.sage.bigdata.etl.plugin.es_2.exception.EsLogWriteException
...
class ElasticSearchWriter(conf: ES2Writer, report: MeterReport) extends DefaultWriter[ES2Writer](conf, report: MeterReport) {
  ...
  class DataStore(real: ElasticSearchWriter) extends Actor with BatchProcess with Mapper {
    ...
    private def createIndex(index: String, `type`: String, numeric_detection: Boolean, date_detection: Boolean, enable_size: Boolean) {
      ...
      client.get().admin().indices().prepareCreate(index).setSettings(settings)
        .addMapping(`type`, XContentFactory.jsonBuilder().startObject().field("numeric_detection", numeric_detection).field("date_detection", date_detection).startObject("_size").field("enabled", enable_size).endObject().endObject())
        .execute().actionGet()
    }
    ...
    def doWork(bulk: BulkRequestBuilder)() {
      ...
      try {
        ...
        case e: Exception =>
          e.printStackTrace()
          logger.debug(s"submit data to elastic has error:${e.getStackTrace}")
          schedule()
      }
    }
    ...
    private def isUsable: Boolean = {
      try {
        if (!connect) {
          ...
          connect = true
          context.parent ! CONNECTED
          logger.info(" connect to elasticsearch 2.x  success!")
        }
      } catch {
        case e: Exception =>
          logger.debug(s"connect to elastic 2.x has error:${e.getMessage}")
          schedule()
      }
      connect
    }
    ...
    def indexRequest(id: String, indices: String, log: Map[String, Any], bulk: BulkRequestBuilder, flag: Boolean = true): BulkRequestBuilder = {
      import scala.collection.JavaConversions._
      ...
      conf.asChild match {
              ...
              log.get(parent) match {
                case Some(v) if v != null =>
                  ...
                  val data = client.get().prepareUpdate(indices, _type, v.toString)
                    .setScript(new Script(script, ScriptType.INLINE, null, Map("param" -> javaMap, "list" -> list)))
                    .setUpsert(indexRequest)
                    .setRetryOnConflict(5)
                  bulk.add(data)
                case _ =>
                  val data = client.get().prepareIndex(indices + "_parent_miss", _type, id).setSource(javaMap)
        ...
        case _ =>
          generateId(log, conf.idFields) match {
            case Some(id: String) if id.trim.length > 0 =>
              var data = try {
                new IndexRequest(indices, _type, id.toString).source(javaMap)
              } catch {
              ...
              var updateRequest = client.get().prepareUpdate(indices, _type, id).setDoc(javaMap)
            ...
            case _ =>
              var data = try {
                client.get().prepareIndex(indices, _type, id).setSource(javaMap)
              } catch {
                case e: ClassCastException =>
                  logger.error(s"index data has error:" + e.getMessage)
