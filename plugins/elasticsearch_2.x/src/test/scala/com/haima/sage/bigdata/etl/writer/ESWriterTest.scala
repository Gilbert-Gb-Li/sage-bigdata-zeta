...
import com.haima.sage.bigdata.etl.common.model.{ES2Writer, Opt, Status, Writer}
import com.haima.sage.bigdata.etl.plugin.es_2.writer.ElasticSearchWriter
...
class ESWriterTest extends Mapper {
  ...
  def test(): Unit = {
    ...
    val writer = system.actorOf(Props(classOf[ElasticSearchWriter], ES2Writer("0", "elasticsearch", Array(("127.0.0.1",9200)), "logs_%{yyyyMMdd}", "iis", cache = 1000), metric.timer("test")), "startActor")
