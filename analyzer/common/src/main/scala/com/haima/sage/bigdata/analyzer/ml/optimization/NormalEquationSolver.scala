...
import com.haima.sage.bigdata.analyzer.ml.linalg.{BLAS, CholeskyDecomposition}
...
/**
  ...
  */
private[optimization] class NormalEquationSolution(
...
/**
  ...
  */
private[optimization] sealed trait NormalEquationSolver {
...
/**
  ...
  */
private[optimization] class CholeskySolver extends NormalEquationSolver {
  ...
  override def solve(
                      ...
                      aBar: DenseVector): NormalEquationSolution = {
    val k = abBar.size
    val x = CholeskyDecomposition.solve(aaBar.data, abBar.data)
    val aaInv = CholeskyDecomposition.inverse(aaBar.data, k)
...
/**
  ...
  */
private[optimization] class QuasiNewtonSolver(
  ...
  private class NormalEquationCostFun(
    ...
    override def calculate(coefficients: BDV[Double]): (Double, BDV[Double]) = {
      val coef =  coefficients.fromBreeze[DenseVector]
      if (fitIntercept) {
        var j = 0
        var dotProd = 0.0
        val coefValues = coef.data
...
/**
  ...
  * (singular).
  */
private[analyzer] class SingularMatrixException(message: String, cause: Throwable)
