...
/**
  ...
  */
private[sage] class WeightedLeastSquaresModel(
...
/**
  ...
  */
private[sage] class WeightedLeastSquares(
                                          ...
                                          val maxIter: Int = 100,
                                          val tol: Double = 1e-6) extends Logger with Serializable {
  ...
  /**
    ...
    */
  def fit(instances: DataSet[Instance]): DataSet[WeightedLeastSquaresModel] = {

    instances.map(d => new Aggregator().add(d)).reduce(_.merge(_)).map(summary => {
      //  instances.treeAggregate(new Aggregator)(_.add(_), _.merge(_))
      summary.validate()
      ...
      if ((rawBStd == 0) && (fitIntercept || rawBBar == 0.0)) {
        if (rawBBar == 0.0) {
        ...
        val aStdValues = aStd.data

        val aBar = {
          val _aBar = summary.aBar
          val _aBarValues = _aBar.data
          var i = 0
        ...
        val aBarValues = aBar.data

        val abBar = {
          val _abBar = summary.abBar
          val _abBarValues = _abBar.data
          var i = 0
        ...
        val abBarValues = abBar.data

        val aaBar = {
          val _aaBar = summary.aaBar
          val _aaBarValues = _aaBar.data
          var j = 0
          var p = 0
        ...
        while (q < len) {
          coefficientArray(q) *= {
            if (aStdValues(q) != 0.0) bStd / aStdValues(q) else 0.0
          }
          q += 1
        }
...
private[sage] object WeightedLeastSquares {
