package com.haima.sage.bigdata.analyzer.streaming.processor
...
import com.haima.sage.bigdata.analyzer.streaming.source.{AkkaSink, AkkaSubscribe}
...
/**
  ...
  * @param writers
  */
class StreamingProcessor(channel: Channel,
                         val analyzer: Analyzer,
                         _cluster: String,
                         val writers: List[ActorRef]
                        )
  ...
  private val jar: List[String] = JarUtils.jar ++ {
    val path = classOf[StreamingProcessor].getClass.getResource("/").getPath
  ...
  class RunFlinkJobActor(remote: ActorRef) extends Actor {
          ...
          case e: ProgramInvocationException =>
            e.getCause.getCause match {
              case ne: NoResourceAvailableException =>
                remote ! (self.path.name, ProcessModel.LEXER, Status.ERROR, s"FLINK COMMIT ERROR:${ExceptionUtils.getMessage(ne)}")
              case e: Throwable =>
                e.printStackTrace()
                remote ! (self.path.name, ProcessModel.LEXER, Status.ERROR, s"FLINK COMMIT ERROR:${ExceptionUtils.getMessage(e)}")
                logger.error("FLINK COMMIT ERROR", e)
              case ex =>
                remote ! (self.path.name, ProcessModel.LEXER, Status.ERROR, s"FLINK COMMIT ERROR:${ExceptionUtils.getMessage(ex)}")
  ...
  private def writerWithDefaultBatch(log: RichMap): Unit = {
    ...
    logger.info(s" flink lexer send data to ")
  ...
  def active(jobId: String, wait: Long, addCluster: Boolean, addTime: Boolean): Receive = apply(jobId.toString).orElse {
    ...
    case (Opt.START, wait: Long, addCluster: Boolean, addTime: Boolean) =>
      if (jobId != null) {
        context.actorOf(Props.create(classOf[RunFlinkJobActor], this, context.parent)) ! Opt.START
        context.unbecome()
  ...
  override def from(channel: SingleChannel, path: String): DataStream[RichMap] = {
    logger.info(s"start subscribe to ${channel.collector.get},${channel.id.get}")

    val source = new AkkaSubscribe(channel.collector.get, channel.id.get, lexer = AkkaUtils.getPath(self), path + "end")
