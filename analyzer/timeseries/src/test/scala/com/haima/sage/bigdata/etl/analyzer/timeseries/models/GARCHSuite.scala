...
class GARCHSuite extends FunSuite {
  test("GARCH log likelihood") {
    ...
    val ts = DenseVector(model.sample(n, rand))
  ...
  test("standardize and filter") {
    ...
    val standardized = model.removeTimeDependentEffects(ts, DenseVector.zeros(n))
    // heteroskedasticize
    val filtered = model.addTimeDependentEffects(standardized, DenseVector.zeros(n))

    assert((filtered.asBreeze - ts.asBreeze).toArray.forall(math.abs(_) < .001))
  }
}
