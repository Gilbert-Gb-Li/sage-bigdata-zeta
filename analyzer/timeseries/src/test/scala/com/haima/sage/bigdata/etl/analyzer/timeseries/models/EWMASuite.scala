...
class EWMASuite extends FunSuite with Matchers {
  test("adding time dependent effects") {
    ...
    val smoothed1 = m1.addTimeDependentEffects(orig, new DenseVector(Array.fill(10)(0.0)))

    smoothed1(0) should be (orig(0))
    smoothed1(1) should be (m1.smoothing * orig(1) + (1 - m1.smoothing) * smoothed1(0))
    round2Dec(vector2Array(smoothed1).last) should be (6.54)

    val m2 = new EWMAModel(0.6)
    val smoothed2 = m2.addTimeDependentEffects(orig,  new DenseVector(Array.fill(10)(0.0)))

    smoothed2(0) should be (orig(0))
    smoothed2(1) should be (m2.smoothing * orig(1) + (1 - m2.smoothing) * smoothed2(0))

    val dd=round2Dec(vector2Array(smoothed2).last)
    dd should be (9.33)
  }
  ...
  test("removing time dependent effects") {
    ...
    val m1 = new EWMAModel(0.2)
    val orig1 = m1.removeTimeDependentEffects(smoothed,  new DenseVector(Array.fill(10)(0.0)))

    round2Dec(orig1(0)) should be (1.0)
    vector2Array(orig1).last.toInt should be(10)
  }
