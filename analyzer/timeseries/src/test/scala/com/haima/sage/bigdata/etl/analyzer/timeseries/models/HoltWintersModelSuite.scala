...
import org.apache.flink.ml.math.{DenseVector, vector2Array}
...
class HoltWintersModelSuite extends FunSuite {
  ...
  test("Optimal Paramaters alpha beta gamma - Additive Model") {
    ...
    val period = 12
    val model = HoltWinters.fitModel(tsAirPassengers, period)
  ...
  test("Forecast - Additive Model") {
    ...
    val forecasted = model.forecast(tsAirPassengers, period)
    ...
    val result=new Array[Double](period)
    for (i <- 0 until period) {
      forecasted(i) should be (actualForecasted(i) +- 10)
      result(i)=forecasted(i)
    }

    EasyPlot.ezplot(tsAirPassengers,new DenseVector(tsAirPassengers.data++result),'-')
    Thread.sleep(10000)
  }
  ...
  test("Optimal Paramaters alpha beta gamma - Multiplicative Model") {
    val period = 12
    val model = HoltWinters.fitModel(tsCO2, period, "multiplicative")
  ...
  test("Forecast - Multiplicative Model") {
    ...
    val forecasted = model.forecast(tsCO2, period)
