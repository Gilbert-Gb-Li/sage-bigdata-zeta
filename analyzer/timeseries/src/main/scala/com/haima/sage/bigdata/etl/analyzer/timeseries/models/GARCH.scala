...
object ARGARCH {
  ...
  def fitModel(ts: Vector): ARGARCHModel = {
    ...
    val residuals = arModel.removeTimeDependentEffects(ts, DenseVector.zeros(ts.size))
...
class GARCHModel(
   ...
   */
  private[timeseries] def gradient(ts: Vector): Array[Double] = {
  ...
  override def forecast(ts: Vector, future: Int): Vector = {
    throw new NotImplementedError("Garch forecast not implemented ")
  }
}
...
class EGARCHModel(
  ...
  override def forecast(ts: Vector, future: Int): Vector = {
    throw new NotImplementedError("EGARCHModel forecast not implemented ")
  }
}
