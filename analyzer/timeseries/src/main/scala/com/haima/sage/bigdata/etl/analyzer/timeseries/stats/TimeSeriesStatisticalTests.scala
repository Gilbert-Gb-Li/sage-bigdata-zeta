...
import com.haima.sage.bigdata.analyzer.timeseries.{Lag, UnivariateTimeSeries}
import org.apache.flink.ml.math._
import com.haima.sage.bigdata.analyzer.ml.utils.MatrixUtils
import com.haima.sage.bigdata.analyzer.ml.utils.MatrixUtils.{fromBreeze, toBreeze}
import com.haima.sage.bigdata.analyzer.timeseries.UnivariateTimeSeries
...
object TimeSeriesStatisticalTests {
  ...
  def adftest(ts: Vector, maxLag: Int, regression: String = "c"): (Double, Double) = {
    ...
    val test1=vector2Array(UnivariateTimeSeries.differencesAtLag(ts, 1))
    val tsDiff = new DenseVector(vector2Array(UnivariateTimeSeries.differencesAtLag(ts, 1)).slice(1, ts.size))
    ...
    if (regression != "nc") {
      val withTrend = MatrixUtils.matToRowArrs(addTrend(lagMat, regression))
      ols.newSampleData(tsdShort.toArray, withTrend)
    } else {
      ols.newSampleData(tsdShort.toArray, MatrixUtils.matToRowArrs(lagMat))
    }
  ...
  def bgtest(residuals: Vector, factors: Matrix, maxLag: Int): (Double, Double) = {
    val origResiduals = residuals.map(_._2).toArray[Double]
    val origFactors = MatrixUtils.matToRowArrs(factors) // X (wiki)
  ...
  def lbtest(residuals: Vector, maxLag: Int): (Double, Double) = {
    ...
    val adjAutoCorrs = autoCorrs.map { case (p, k) =>
      (p * p) / (n - k - 1)
    }
  ...
  def bptest(residuals: Vector, factors: Matrix): (Double, Double) = {
    val residualsSquared = residuals.valueIterator.map(x => x * x).toArray // u^2
    val origFactors = MatrixUtils.matToRowArrs(factors) // X
  ...
  def kpsstest(ts: Vector, method: String): (Double, Map[Double, Double]) = {
    ...
    val ols = new OLSMultipleLinearRegression()
    ols.setNoIntercept(true)
    ols.newSampleData(ts.valueIterator.toArray, regressors)
