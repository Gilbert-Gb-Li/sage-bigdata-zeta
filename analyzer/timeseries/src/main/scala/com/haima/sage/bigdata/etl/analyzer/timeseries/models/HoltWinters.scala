...
object HoltWinters {
  ...
  def fitModelWithBOBYQA(ts: Vector, period: Int, modelType: String): HoltWintersModel = {
    ...
    val objectiveFunction = new ObjectiveFunction(
      new MultivariateFunction {
        override def value(params: Array[Double]): Double =
...
class HoltWintersModel(
  ...
  def sse(ts: Vector): Double = {
    val n = ts.size
    val smoothed = addTimeDependentEffects(ts, new DenseVector(Array.fill(n)(0.0)))
  ...
  def forecast(ts: Vector, future: Int): Vector = {
    ...
    val finalSeason = (0 until period).map(i => season(i + n - period))
    val destArr = (0 until future).map(i => if (additive) {
      (finalLevel + (i + 1) * finalTrend) + finalSeason(i % period)
    } else {
      (finalLevel + (i + 1) * finalTrend) * finalSeason(i % period)
    }).toArray

    DenseVector(ts.valueIterator.toArray ++ destArr)
  }
  ...
  def getHoltWintersComponents(ts: Vector): (Vector, Vector, Vector, Vector) = {
    ...
    level(0) = initLevel
    trend(0) = initTrend
    initSeason.indices.foreach(i => season(i) = initSeason(i))

    (0 until n - period).foreach(i => {
      dest(i + period) = level(i) + trend(i)

      // Add the seasonal factor for additive and multiply for multiplicative model.
      if (additive) dest(i + period) += season(i) else dest(i + period) *= season(i)

      val levelWeight = if (additive) ts(i + period) - season(i) else ts(i + period) / season(i)

      level(i + 1) = alpha * levelWeight + (1 - alpha) * (level(i) + trend(i))

      trend(i + 1) = beta * (level(i + 1) - level(i)) + (1 - beta) * trend(i)

      val seasonWeight = if (additive) ts(i + period) - level(i + 1) else ts(i + period) / level(i + 1)
      season(i + period) = gamma * seasonWeight + (1 - gamma) * season(i)
    })

    (DenseVector(dest), DenseVector(level), DenseVector(trend), DenseVector(season))
  }
