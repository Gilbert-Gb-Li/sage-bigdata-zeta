...
import com.haima.sage.bigdata.analyzer.ml.utils.MatrixUtils.{dvBreezeToSpark, toBreeze}
import com.haima.sage.bigdata.analyzer.timeseries.UnivariateTimeSeries.{differencesOfOrderD, inverseDifferencesOfOrderD}
import com.haima.sage.bigdata.analyzer.timeseries.stats.TimeSeriesStatisticalTests.kpsstest

import scala.util.{Failure, Try}
import scala.collection.mutable.HashSet

/**
  * Created by CaoYong on 2017/10/23.
  */
/**
...
object ARIMA {
  ...
  def fitModel(
    ...
    val diffedTs = differencesOfOrderD(ts, d).valueIterator.toArray.drop(d)
  ...
  def autoFit(ts: Vector, maxP: Int = 5, maxD: Int = 2, maxQ: Int = 5): ARIMAModel = {
    ...
    if (bestModel == null) {
      /*0, 0, 0, Array(0.0), false*/
      new ARIMAModel(0, d, 0, Array(0.0), false)
    } else {
...
class ARIMAModel(
  ...
  def logLikelihoodCSS(y: Vector): Double = {
    val diffedY = differencesOfOrderD(y, d).valueIterator.toArray.drop(d)
  ...
  def removeTimeDependentEffects(ts: Vector, destTs: Vector): Vector = {
    ...
    val diffedExtended = new BreezeDenseVector[Double](
      (Array.fill[Double](maxLag)(interceptAmt) ++ diffed.valueIterator).toArray)
  ...
  def addTimeDependentEffects(ts: Vector, destTs: Vector): Vector = {
    ...
    val testChanges = inverseDifferencesOfOrderD(changes(maxLag to -1), d)
  ...
  /**
    ...
    *               average terms.
    * @param future Periods in the future to forecast (beyond length of ts)
  ...
  def forecast(ts: Vector, future: Int): Vector = {
    ...
    val diffedTs = new DenseVector(differencesOfOrderD(ts, d).valueIterator.toArray.drop(d))

    // Assumes prior AR terms are equal to model intercept
    val interceptAmt = if (hasIntercept) coefficients(0) else 0.0
    val diffedTsExtended = new DenseVector((Array.fill(maxLag)(interceptAmt) ++ vector2Array(diffedTs)))
    ...
    val forward = new BreezeDenseVector[Double](Array.fill(future + maxLag)(0.0))
    ...
    val results = new BreezeDenseVector[Double](Array.fill(ts.size + future)(0.0))
    ...
    if (d != 0) {
      ...
      // copy into results
      results(-(d + future) to -1) := toBreeze(forwardIntegrated)
    }
    results
  }
