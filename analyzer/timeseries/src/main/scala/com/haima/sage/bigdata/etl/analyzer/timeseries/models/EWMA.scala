...
class EWMAModel(val smoothing: Double) extends TimeSeriesModel {
  ...
  /**
    ...
    */
  private[timeseries] def sse(ts: Vector): Double = {
    val n = ts.size
  ...
  /**
    ...
    * @return gradient
    */
  private[timeseries] def gradient(ts: Vector): Double = {
    val n = ts.size
    val smoothed = new DenseVector(Array.fill(n)(0.0))
    addTimeDependentEffects(ts, smoothed)
  ...
  override def forecast(ts: Vector, future: Int): Vector = {
    throw new NotImplementedError("EWMA forecast not implemented ")
  }
}
