...
import com.haima.sage.bigdata.analyzer.timeseries.models.{ARModel, Autoregression}
...
object UnivariateTimeSeries {
  ...
  def autocorr(ts: Array[Double], numLags: Int): Array[Double] = {
    autocorr(new DenseVector(ts), numLags).map(_._2).toArray[Double]
  }
  ...
  def autocorr(ts: Vector, numLags: Int): Vector = {
    ...
    var i = 1
    val breezeTs = Breeze.Vector2BreezeConverter(ts).asBreeze
    while (i <= numLags) {
  ...
  def trimLeading(ts: Vector): Vector = {
    ...
    if (start < ts.size) {
      VectorBuilder.vectorBuilder.build(Arrays.copyOfRange(ts.valueIterator.toArray, start, ts.size).toList)
    } else {
      VectorBuilder.vectorBuilder.build(List.empty[Double])
    }
  }
  ...
  def trimTrailing(ts: Vector): Vector = {
    ...
    if (end > 0) {
      VectorBuilder.vectorBuilder.build(Arrays.copyOfRange(ts.valueIterator.toArray, 0, end).toList)
    } else {
      VectorBuilder.vectorBuilder.build(List.empty[Double])
    }
  }
  ...
  def fillValue(values: Array[Double], filler: Double): Array[Double] = {
    fillValue(new DenseVector(values), filler).map(_._2).toArray[Double]
  }
  ...
  def fillValue(values: Vector, filler: Double): DenseVector = {
    val result = values.copy.map(_._2).toArray[Double]
    var i = 0
    while (i < result.size) {
  ...
  def fillNearest(values: Array[Double]): Array[Double] = {
    fillNearest(new DenseVector(values)).map(_._2).toArray[Double]
  }
  ...
  def fillPrevious(values: Array[Double]): Array[Double] = {
    fillPrevious(new DenseVector(values)).map(_._2).toArray[Double]
  }
  ...
  def fillNext(values: Array[Double]): Array[Double] = {
    fillNext(new DenseVector(values)).map(_._2).toArray[Double]
  }
  ...
  def fillWithDefault(values: Array[Double], filler: Double): Array[Double] = {
    fillWithDefault(new DenseVector(values), filler).map(_._2).toArray[Double]
  }
  ...
  def fillLinear(values: Array[Double]): Array[Double] = {
    fillLinear(new DenseVector(values)).map(_._2).toArray[Double]
  }
  ...
  def fillSpline(values: Array[Double]): Array[Double] = {
    fillSpline(new DenseVector(values)).map(_._2).toArray[Double]
  }
  ...
  def fillSpline(values: Vector): DenseVector = {
    ...
    val knotsAndValues = values.map(_._2).toArray[Double].zipWithIndex.filter(!_._1.isNaN)
  ...
  def rollSum(ts: Vector, n: Int): Vector = {
    new DenseVector(ts.map(_._2).toArray.sliding(n).toList.map(_.sum).toIndexedSeq.toArray[Double])
  }
