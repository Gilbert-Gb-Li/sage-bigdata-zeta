...
import org.apache.flink.ml.math.{DenseVector, Vector, vector2Array}
import org.apache.flink.ml.math.Breeze._
import com.haima.sage.bigdata.analyzer.timeseries.UnivariateTimeSeries.{differencesOfOrderD, inverseDifferencesOfOrderD}
/**
...
object ARIMAX {
  ...
  def fitModel(
    ...
    val differentialTs = vector2Array(differencesOfOrderD(ts, d)).drop(d)
  ...
  private def estimateARXCoefficients(
    ...
    val tsVector = new BreezeDenseVector(vector2Array(ts))
    val xregVector = new BreezeDenseVector(xreg.toDenseMatrix.data)
    val differentialXregVector = new BreezeDenseVector(vector2Array(differencesOfOrderD(xregVector.fromBreeze, d)))
...
class ARIMAXModel(
  ...
  def forecast(timeSeries: BreezeDenseVector[Double], xreg: Matrix[Double]): BreezeDenseVector[Double] = {
    ...
    val differentialTs = new DenseVector(vector2Array(differencesOfOrderD(ts, d)).drop(d))
    val differentialTsExtended = new DenseVector(Array.fill(maxLag)(intercept) ++ differentialTs.data)
    ...
    iterateARMAX(forward.fromBreeze, forward, _ + _, goldStandard = forward.fromBreeze, initMATerms = maTerms, exogenousVar = differentialXreg)
    ...
    results(0 until d) := ts.asBreeze(0 until d)
    ...
    if (d != 0) {
      ...
      differentialMatrix(0, ::) := ts.asBreeze.t
      for (i <- 1 to d) {
        differentialMatrix(i, i to -1) := differencesOfOrderD(differentialMatrix(i - 1, i to -1).t.fromBreeze, 1).asBreeze.t
      }
      ...
      val forwardIntegrated = inverseDifferencesOfOrderD(
        BreezeDenseVector.vertcat(prevTermsForForwardInverse, forward(maxLag to -1)).fromBreeze,
        d
      )
      results(-(d + nFuture) to -1) := forwardIntegrated.asBreeze
    }
  ...
  private def iterateARMAX(
    ...
    val xregPredictors = AutoregressionX.assemblePredictors(Array.fill(xregMaxLag)(0.0) ++ vector2Array(ts), MatrixUtils.matToRowArrs(xregMatrix), 0, xregMaxLag, includeOriginalXreg)
  ...
  def differenceXreg(xreg: Matrix[Double]): Array[Array[Double]] = {
    ...
    for (i <- 0 until xregColumns) {
      val xregCol = new BreezeDenseVector(xreg.toDenseMatrix.toArray.slice(xreg.rows * i, (i + 1) * xreg.rows))
      val diffXregCol = vector2Array(new DenseVector(Array.fill(math.max(p, q))(0.0) ++ vector2Array(differencesOfOrderD(xregCol.fromBreeze, d)).drop(d)))
  ...
  def removeTimeDependentEffects(ts: Vector, destTs: Vector): Vector = {
    ...
    val diffedExtended = new BreezeDenseVector[Double](
      Array.fill(maxLag)(interceptAmt) ++ vector2Array(diffed))
    ...
    iterateARMA(diffedExtended.fromBreeze, changes, _ - _, errors = changes.fromBreeze)
    val breezeDestTs = destTs.asBreeze
    breezeDestTs := changes(maxLag to -1)
    destTs
  }
  ...
  def addTimeDependentEffects(ts: Vector, destTs: Vector): Vector = {
    ...
    val changes = new BreezeDenseVector[Double](Array.fill(maxLag)(interceptAmt) ++ vector2Array(ts))
    // ts corresponded to errors, and we want to use them
    val errorsProvided = changes.copy
    iterateARMA(changes.fromBreeze, changes, _ + _, errors = errorsProvided.fromBreeze)
    // drop assumed AR terms at start and perform any inverse differencing required
    destTs.asBreeze := inverseDifferencesOfOrderD(changes(maxLag to -1).fromBreeze, d).asBreeze
    destTs
  }

  override def forecast(ts: Vector, future: Int): Vector = {
    throw new NotImplementedError("ArimaX forecast not implemented ")
  }
}