...
class Script {
  ...
  val process: RichMap => RichMap = event => {
    ...
    val Is_license_vague = event("Is_license_vague"); //许可证号是否模糊
              ...
              if (x.getTime < curdate.getTime) { //证过期
                ...
                if (has_license.isEmpty) { //没上传许可证
                  ...
                  if (issue_date.getTime > issue_time.getTime) { //2018年1月1日后颁发
                    //主营业态是否有网络
                    event.get("ocr_business_scope") match {
                      case Some(x:String) if x.toInt!=0=>
                        func2();
