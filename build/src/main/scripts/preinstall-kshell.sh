...
function installPlugins {
    ...
    ES_COUNT=`echo $WORKER_PLUGINS|awk -F 'es[2,5]' '{print NF-1}'`
...
function installMaster {
	...
	rm -rf $MASTER_DEST_DIR/$MASTER_PKG_FOLDER.back 2>/dev/null
        ...
        perl -pi -e "s/akka.remote.netty.tcp.hostname *= *127.0.0.1/akka.remote.netty.tcp.hostname = $MASTER_HOST_INTERNAL/g" $MASTER_DEST_DIR/master/conf/master.conf
        perl -pi -e "s/akka.remote.netty.tcp.port *= *19091/akka.remote.netty.tcp.port = $MASTER_PORT_INTERNAL/g" $MASTER_DEST_DIR/master/conf/master.conf
	    TMPSTR=`echo $MASTER_CLUSTER_SEED_NODES | sed "s/\//\\\\\\\\\//g"`
	    perl -pi -e "s/akka.cluster.seed-nodes *= *\[\"akka.tcp:\/\/master@127.0.0.1:19091\"\]/akka.cluster.seed-nodes = $TMPSTR/g" $MASTER_DEST_DIR/master/conf/master.conf
        perl -pi -e "s/master.web.http.host *= *0.0.0.0/master.web.http.host = $MASTER_WEB_HOST/g" $MASTER_DEST_DIR/master/conf/master.conf
        perl -pi -e "s/master.web.http.port *= *19090/master.web.http.port = $MASTER_WEB_PORT/g" $MASTER_DEST_DIR/master/conf/master.conf
        ...
        perl -pi -e "s/master.web.auth.host *= *127.0.0.1/master.web.auth.host = $AUTH_HOST/g" $MASTER_DEST_DIR/master/conf/master.conf
        perl -pi -e "s/master.web.auth.port *= *9000/master.web.auth.port = $AUTH_PORT/g" $MASTER_DEST_DIR/master/conf/master.conf
    fi
}
...
function installWorker {
	...
	rm -rf $WORKER_DEST_DIR/worker 2>/dev/null
        ...
        perl -pi -e "s/akka.remote.netty.tcp.hostname *= *127.0.0.1/akka.remote.netty.tcp.hostname = $WORKER_HOST/g" $WORKER_DEST_DIR/worker/conf/worker.conf
        perl -pi -e "s/akka.remote.netty.tcp.port *= *19093/akka.remote.netty.tcp.port = $WORKER_PORT/g" $WORKER_DEST_DIR/worker/conf/worker.conf
        perl -pi -e "s/worker.id *= *#workerid#/worker.id = $WORKER_ID/g" $WORKER_DEST_DIR/worker/conf/worker.conf
	    TMPSTR=`echo $MASTER_CLUSTER_SEED_NODES | sed "s/\//\\\\\\\\\//g"`
	    perl -pi -e "s/worker.remote *= *\[\"akka.tcp:\/\/master@127.0.0.1:19091\"\]/worker.remote = $TMPSTR/g" $WORKER_DEST_DIR/worker/conf/worker.conf
        perl -pi -e "s/worker.daemon.host *= *127.0.0.1/worker.daemon.host = $WORKER_HOST/g" $WORKER_DEST_DIR/worker/conf/worker.conf
        perl -pi -e "s/worker.daemon.port *= *19095/worker.daemon.port = $DAEMON_PORT/g" $WORKER_DEST_DIR/worker/conf/worker.conf
        perl -pi -e "s/worker.daemon.enable *= *on/worker.daemon.enable = $DAEMON_ENABLED/g" $WORKER_DEST_DIR/worker/conf/worker.conf

	    #install worker will also install and config daemon
        perl -pi -e "s/akka.remote.netty.tcp.hostname *= *127.0.0.1/akka.remote.netty.tcp.hostname = $WORKER_HOST/g" $WORKER_DEST_DIR/worker/conf/daemon.conf
        perl -pi -e "s/akka.remote.netty.tcp.port *= *19095/akka.remote.netty.tcp.port = $DAEMON_PORT/g" $WORKER_DEST_DIR/worker/conf/daemon.conf
    fi
}
...
function resetDB {
    perl -pi -e "s/#ES_CLUSTER#/$ES_CLUSTER/g" $MASTER_DEST_DIR/master/bin/insert.sql
    perl -pi -e "s/#ES_HOSTPORTS#/$ES_HOSTPORTS/g" $MASTER_DEST_DIR/master/bin/insert.sql
    perl -pi -e "s/#SYSLOG_HOST#/$SYSLOG_HOST/g" $MASTER_DEST_DIR/master/bin/insert.sql
    perl -pi -e "s/#SYSLOG_PORT#/$SYSLOG_PORT/g" $MASTER_DEST_DIR/master/bin/insert.sql
    perl -pi -e "s/#WRITER_KAFKA_HOSTPORTS#/$WRITER_KAFKA_HOSTPORTS/g" $MASTER_DEST_DIR/master/bin/insert.sql
    perl -pi -e "s/#WRITER_KAFKA_TOPIC#/$WRITER_KAFKA_TOPIC/g" $MASTER_DEST_DIR/master/bin/insert.sql
    perl -pi -e "s/#DATASOURCE_KAFKA_HOSTPORTS#/$DATASOURCE_KAFKA_HOSTPORTS/g" $MASTER_DEST_DIR/master/bin/insert.sql
    perl -pi -e "s/#DATASOURCE_KAFKA_TOPIC#/$DATASOURCE_KAFKA_TOPIC/g" $MASTER_DEST_DIR/master/bin/insert.sql
    perl -pi -e "s/#OMDB_HOST#/$OMDB_HOST/g" $MASTER_DEST_DIR/master/bin/insert.sql
    perl -pi -e "s/#OMDB_PORT#/$OMDB_PORT/g" $MASTER_DEST_DIR/master/bin/insert.sql
    perl -pi -e "s/#OMDB_SCHEMA#/$OMDB_SCHEMA/g" $MASTER_DEST_DIR/master/bin/insert.sql
    perl -pi -e "s/#FLINK_HOSTPORTS#/$FLINK_HOSTPORTS/g" $MASTER_DEST_DIR/master/bin/insert.sql
    perl -pi -e "s/#OMDB_USER#/$OMDB_USER/g" $MASTER_DEST_DIR/master/bin/insert.sql
    perl -pi -e "s/#OMDB_PWD#/$OMDB_PWD/g" $MASTER_DEST_DIR/master/bin/insert.sql
    TMPSTR=`echo $MASTER_DEST_DIR | sed "s/\//\\\\\\\\\//g"`
    perl -pi -e "s/#MASTER_DATASOURCE#/$TMPSTR\/master\/logs\/master-.*\.log/g" $MASTER_DEST_DIR/master/bin/insert.sql
    perl -pi -e "s/#MASTER_PARSER_WRITER#/$TMPSTR\/master\/logs\/master-parser.log/g" $MASTER_DEST_DIR/master/bin/insert.sql
    perl -pi -e "s/#MASTER_ANALYZER_WRITER#/$TMPSTR\/master\/logs\/master-analyzer.log/g" $MASTER_DEST_DIR/master/bin/insert.sql
