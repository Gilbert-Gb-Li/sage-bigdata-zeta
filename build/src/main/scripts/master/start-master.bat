...
:cont
REM ***** Replace master.conf path *****
Set confFile=%SAGE_HOME%\conf\master.conf
Set confFileTmp=%SAGE_HOME%\conf\master.conf.tmp
Set confStr=./db/
...
REM ***** Replace logback-master.xml path *****
Set logFile=%SAGE_HOME%\conf\logback-master.xml
Set logFileTmp=%SAGE_HOME%\conf\logback-master.xml.tmp
Set logStr=logs/
...
SETLOCAL
TITLE SAGE-master
"%JAVA_HOME%\bin\java" %JAVA_OPTS% %SAGE_JAVA_OPTS% %SAGE_PARAMS% %* -cp "%SAGE_CLASSPATH%" "com.haima.sage.bigdata.etl.server.Master"
