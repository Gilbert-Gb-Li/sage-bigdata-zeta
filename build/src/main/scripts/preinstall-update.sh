...
sage-bigdata-etl_PKG_FOLDER=${sage-bigdata-etl_BN/.tar.gz}
SYS_VERSION=`echo $sage-bigdata-etl_PKG_FOLDER|awk -F"-" '{print $2}'`

case $SYS_VERSION in
(*nix*) sage-bigdata-etl_VERSION=${sage-bigdata-etl_PKG_FOLDER/sage-bigdata-etl-nix-};;
(*aix*) sage-bigdata-etl_VERSION=${sage-bigdata-etl_PKG_FOLDER/sage-bigdata-etl-aix-};;
(*hpux*) sage-bigdata-etl_VERSION=${sage-bigdata-etl_PKG_FOLDER/sage-bigdata-etl-hpux-};;
esac

sage-bigdata-etl_PKG_TAR=${sage-bigdata-etl_BN/.gz}
    ...
    then
	    ...
	    then
            ...
            then
                ...
                sed -i $SEDOPT "s/master.web.auth.enable *= *on/master.web.auth.enable = off/g" $MASTER_DEST_DIR/master/conf/master.conf
                sed -i $SEDOPT "s/var auth *= *\!0/var auth=\!$AUTH_ENABLED/g" $MASTER_DEST_DIR/master/conf/ui/scripts/app.min.js
            else
                sed -i $SEDOPT "s/master.web.auth.enable *= *off/master.web.auth.enable = on/g" $MASTER_DEST_DIR/master/conf/master.conf
                sed -i $SEDOPT "s/var auth *= *\!1/var auth=\!$AUTH_ENABLED/g" $MASTER_DEST_DIR/master/conf/ui/scripts/app.min.js
                sed -i $SEDOPT "s/master.web.auth.enable *= *on/master.web.auth.enable = on/g" $MASTER_DEST_DIR/master/conf/master.conf
                sed -i $SEDOPT "s/var auth *= *\!0/var auth=\!$AUTH_ENABLED/g" $MASTER_DEST_DIR/master/conf/ui/scripts/app.min.js
            fi
	    else
