...
import porter.util.{AES, Base64, Hash}
...
case class DerivedCredentials private (accountName: Ident, secret: Secret, valid: Long) extends AccountCredentials {
  ...
  def isExpired: Boolean = valid > 0 && valid < System.currentTimeMillis()
...
object DerivedCredentials {
  ...
  def tryDecode(key: Vector[Byte], data: String): Try[DerivedCredentials] = decode(key, data).map {
