...
class PorterAkkaClient(val porterRef: PorterRef, val decider: Decider) extends PorterClient {
  ...
  private def exec[A, B: ClassTag] = new Command[A, B] {
    override def apply(req: A)(implicit ec: ExecutionContext, timeout: FiniteDuration, manifest: Manifest[B]): Future[B] = {
  ...
  def authenticateAccount = new AuthcAccountCmd {
    def apply(req: Authenticate)(implicit ec: ExecutionContext, timeout: FiniteDuration, manifest: Manifest[AuthAccount]): Future[AuthAccount] = {
      implicit val to: Timeout = Timeout(timeout)
  ...
  def authenticateSimple(realm: Ident) = new AuthcSimpleCmd {
    def apply(req: UserPass)(implicit ec: ExecutionContext, timeout: FiniteDuration, manifest: Manifest[SimpleAuthResult]): Future[SimpleAuthResult] = {
      implicit val to: Timeout = Timeout(timeout)
  ...
  def authorize: Command[Authorize, AuthorizeResp] {
    def apply(req: Authorize)
             (implicit ec: ExecutionContext, timeout: FiniteDuration, manifest: Manifest[AuthorizeResp]): Future[AuthorizeResp]
  } = exec[Authorize, AuthorizeResp]
  ...
  def createNewAccount = new UpdateAccountCmd {
    def apply(req: UpdateAccount)(implicit ec: ExecutionContext, timeout: FiniteDuration, manifest: Manifest[OperationFinished]): Future[OperationFinished] = {
      implicit val to: Timeout = Timeout(timeout)
  ...
  def changeSecrets = new ChangeSecretsCmd {
    def apply(req: ChangeSecrets)(implicit ec: ExecutionContext, timeout: FiniteDuration, manifest: Manifest[OperationFinished]): Future[OperationFinished] = {
      implicit val to: Timeout = Timeout(timeout)
