...
class PorterMain(settings: PorterSettings) extends Actor with ActorLogging {
  ...
  def receive = {
    case ShowSettings =>
      sender ! settings.toString
    case sm: StoreMessage =>
      store forward sm
    case mm: MutableStoreMessage =>
      mstore forward mm
    case mr: MakeRules =>
      ruleFactory forward mr
    case gp: GetPolicy =>
      policy forward gp
    case authz: Authorize =>
      log.debug("authz:"+authz)
      policy forward authz
    case authc: Authenticate =>

      log.debug("authc:"+authc)
