...
class DigestHa1ValidatorTest extends FunSuite with Matchers {
  ...
  def ha1(username: String, password: String): String = Hash.md5String(username +":"+ realm.name +":"+ password)

  def clientResponse(ha1: String, nonce: String, qop: Option[DigestQop]): String = {
    val ha2 = qop match {
