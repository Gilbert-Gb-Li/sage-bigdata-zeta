...
class MongoStoreTest extends FunSuite with Matchers with BeforeAndAfterAll {
  ...
  override protected def afterAll() = {
    val client = MongoStore.createMongo(MongoDriver(), config)
    Await.ready(Await.result[DefaultDB](client.database(config.getString("dbname")),5.seconds).drop(), 5.seconds)
    system.terminate()
  }
